---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

### Research Interests

Robotic manipulation and grasping. Reinforcement Learning (RL) for Vision-Language-Action (VLA) models.

### Education

- **Ph.D. Candidate in Robotics and Smart Manufacturing**, Zhongguancun Academy & SUSTech (Expected 2027)  
  Advisors: Prof. Hong Zhang, Prof. Yue Wang
- **M.S. in Electronic Science and Technology**, SUSTech (2022–2024)  
  Advisor: Prof. Hong Zhang
- **B.E. in Information Engineering**, South China Normal University (2018–2022)

### Selected Publications

- **Huang, Dehao**, Wenlong Dong, Chao Tang, Hong Zhang. *HGDiffuser: Efficient Task-Oriented Grasp Generation via Human-Guided Grasp Diffusion Models.* IROS 2025.  
  [Paper](https://arxiv.org/pdf/2503.00508) · [Video](https://sites.google.com/view/hgdiffuser) · [Code](https://github.com/red0orange/handgrasp_ws)
- **Huang, Dehao**, Chao Tang, Hong Zhang. *Efficient Object Rearrangement via Multi-view Fusion.* ICRA 2024.  
  [Paper](https://arxiv.org/pdf/2309.08994) · [Video](https://sites.google.com/view/multi-view-rearr)
- Shao, Chenyang, **Huang, Dehao**, et al. *OmniScientist: Toward a Co-evolving Ecosystem of Human and AI Scientists.* arXiv 2025.  
  [Paper](https://arxiv.org/pdf/2511.16931)

### Competitions

- **RoboMaster Robotics Competition** (Algorithm Group Leader), 2019–2021  
  Enhanced armor recognition; monocular-vision 3D positioning (PnP); ROS debugging tools; AprilTag localization and control.
- **National College Students' Smart Car Competition** (Team Leader), 2020  
  Ackerman model optimization; EKF fusion (odometer + IMU) for accuracy.

### Awards

- 1st Academic Research Star of Zhongguancun Academy (2026)
- RoboMaster National Second Prize (2020)
- American ICM / MCM Meritorious Winner (2021)
- SCNU First-Class Scholarship (2020)
- National Undergraduate Smart Car Competition Third Prize (2020)

### Skills

- **Programming**: Python, C/C++
- **Tools**: PyTorch, ROS, Git, LaTeX
- **Languages**: Chinese (native), English (CET-6)
